...
| Phones cannot see each other | Both must be in the *same group* with the app open, and Bluetooth on. Give it a few seconds. Arming is *not* required to appear in each other's group list. |
| The group list flickers, or names never appear | Almost always the **Saver** power profile: it listens about a tenth of the time, so peers update every few seconds rather than continuously. **Balanced** (the default since 1.6.0) or **Maximum** settles it. |
| "Arm all" only armed some phones | Check the count under **Group** — that is the real answer, and it is the one thing nothing can fake. If a phone is genuinely missed, press it again; commands are repeated for half a minute and relayed by every phone that hears them, so a retry only has work to do when the first one truly did not land. |
| One phone stopped alarming, the others did not | Use **Stop the alarm** or **Stop and disarm everyone**, on any phone, including one that has already gone quiet. Both reach the whole group, from the lock-screen prompt, the notification, or the app. Since 1.6.0 the alarm panel stays on screen while *anybody* in the group is still alarming. |
| "The group is still alarming" appears when nothing is | Fixed in 1.6.1: a silenced phone used to go on broadcasting the alarm for the whole command repeat window, so everyone else kept hearing an incident that was over. If you still see it, some phone genuinely has not stopped — press **Stop the alarm** again. |
| A phone drops off the group list after an alarm | Fixed in 1.6.1. Dealing with an alarm from the lock screen removed the alarm surface's task, which Android reports to the app's services as "the user closed the app", so the guard shut down on that phone until somebody reopened it. |
